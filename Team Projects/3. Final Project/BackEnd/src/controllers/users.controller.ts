...
export class UsersController {
    ...
    @Get(':userId')
    async getUserByname(@Param('userId') userId: string) {
        return await this.usersService.nextWeekStatus(+userId);
    }


    @Put(':userId')
    async assignProjectToUser(@Param('userId') userId: string, @Body() projectId: ProjectIdDTO) {
        return await this.usersService.assignProjectToUser(+userId, projectId);
    }

    @Get(':userId/vacations')
    async userVacations(@Param('userId') userId: string) {
        return await this.usersService.userVacations(+userId);
    }


    @Post(':userId/vacations')
    async createNewVacation(@Param('userId') userId: string, @Body() vacationsInfo: CreateVacationDTO) {
        return await this.usersService.createNewVacation(+userId, vacationsInfo);
    }

    @Delete(':userId/vacations/:vacationId')
    async deleteVacation(@Param('vacationId') vacationId: string, @Param('userId') userId: string) {
        return await this.usersService.deleteVacation(+vacationId, +userId);
    }
